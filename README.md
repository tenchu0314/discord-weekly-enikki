...
- Botが所属する **全サーバーの全テキスト・フォーラムチャンネル** から会話を収集
...
```text
1. Discordにログイン
2. 全サーバー・全テキストチャンネルおよびフォーラムチャンネルから過去1週間のメッセージを収集
   - Bot投稿は除外
   - フォーラムはアクティブ・アーカイブ済みスレッドを両方対象
   - レート制限対策のウェイト付き
...
- Discord APIにはレート制限があります。大量のチャンネルやフォーラムスレッドがある場合、取得に時間がかかることがあります
